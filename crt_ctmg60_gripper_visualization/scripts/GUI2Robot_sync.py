...
l4 = [1001,1001]
l5 = [0 for i in range(5)]
l6 = [1001,1001]
def callback(data,args):
    # print(data)
    rate = rospy.Rate(50)
    p1 = int((0.85 - data.position[0]) / 0.85 * 100)
    # p2 = int((0.85 - data.position[2]) / 0.85 * 100)
    # p3 = int((0.85 + data.position[4]) / 0.85 * 100)
    # global l1,l2
    # l1.append(p1)
    # l1.pop(0)
    # if l1.count(p1) >=3:
    ...
    l1.append(p1)
    if l1.count(p1) >= 3: 
      write_register(args,FINGER_1_POSITION_REGISTER,p1)   #传入位置信息
    #print("机械指位置：{:f} ".format(data.position[1]))
    rate.sleep()


if __name__ == '__main__':
    print("程序开始")
    write_register(instrument,SYNCHRONIZATION_REGISTER,1)   #设置同步
